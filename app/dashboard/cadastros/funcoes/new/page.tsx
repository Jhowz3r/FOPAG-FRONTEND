...
export default function NewFuncaoPage() {
  ...
  const pisoSalarial = watch("pisoSalarial");
  const tetoSalarial = watch("tetoSalarial");
  ...
  const loadCbos = async () => {
    try {
      // Tentar buscar CBOs do backend, se não existir, usar array vazio
  ...
  const onSubmit = async (data: FuncaoFormData) => {
    try {
      ...
      setSuccessMessage("Função cadastrada com sucesso!");
      setTimeout(() => {
        router.push("/dashboard/cadastros/funcoes");
      }, 2000);
    } catch (error: any) {
      setErrorMessage(
        error.response?.data?.message || "Erro ao salvar função"
      );
    }
  };
