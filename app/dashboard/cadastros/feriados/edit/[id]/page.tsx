...
export default function EditFeriadoPage() {
  ...
  const loadFeriado = async (id: number) => {
    try {
      setLoading(true);
      ...
      const dataFeriado = feriado.dataFeriado ? new Date(feriado.dataFeriado).toISOString().split('T')[0] : "";
      
      reset({
        idFilial: feriado.idFilial || undefined,
        dataFeriado: dataFeriado,
        descricao: feriado.descricao,
        tipoFeriado: feriado.tipoFeriado,
        idMunicipio: feriado.idMunicipio || undefined,
        tipoAdicaoFeriado: feriado.tipoAdicaoFeriado || undefined,
        feriadoParcial: feriado.feriadoParcial || false,
        horaInicio: feriado.horaInicio || undefined,
        horaFinal: feriado.horaFinal || undefined,
        descontaFeriasColetivas: feriado.descontaFeriasColetivas || false,
        descontaFerias: feriado.descontaFerias || false,
      });
    } catch (error: any) {
      setErrorMessage("Erro ao carregar feriado");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (data: FeriadoFormData) => {
    try {
      ...
      await api.patch(`/feriados/${feriadoId}`, payload);
      ...
      setErrorMessage(
        error.response?.data?.message || "Erro ao atualizar feriado"
      );
    }
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Editar Feriado</h1>
        </div>
      </div>
