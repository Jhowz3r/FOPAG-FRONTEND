...
export default function EditDepartamentoPage() {
  ...
  useEffect(() => {
    loadDivisoes();
    if (departamentoId) {
      loadDepartamento(Number(departamentoId));
    }
  }, [departamentoId]);
  ...
  const loadDepartamento = async (id: number) => {
    try {
      setLoading(true);
      const response = await api.get(`/departamentos/${id}`);
      const departamento = response.data;
      reset({
        idDivisao: departamento.idDivisao,
        codigo: departamento.codigo,
        descricao: departamento.descricao,
      });
    } catch (error: any) {
      setErrorMessage("Erro ao carregar departamento");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (data: DepartamentoFormData) => {
    try {
      ...
      await api.patch(`/departamentos/${departamentoId}`, payload);
      ...
      setErrorMessage(
        error.response?.data?.message || "Erro ao atualizar departamento"
      );
    }
  };
