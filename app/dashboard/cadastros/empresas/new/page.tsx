...
export default function NewEmpresaPage() {
  ...
  const loadTabelasEvento = async () => {
    try {
      // Tentar buscar tabelas de eventos
      // Se não existir endpoint, usar valor padrão
      const response = await api.get("/tabela-eventos");
      setTabelasEvento(response.data);
    } catch (error) {
      // Usar valor padrão se não houver endpoint
  ...
  const onSubmit = async (data: EmpresaFormData) => {
    try {
      ...
      setTimeout(() => {
        router.push("/dashboard/cadastros/empresas");
      }, 2000);
    } catch (error: any) {
      setErrorMessage(
